...
func doAuth(req *AuthReq, r *http.Request) (replyStr *string, err error) {
	...
	*/
	...
	if strings.Contains(recvMsg.Content, "注册") {
		...
		replyMsg.Content = getContent(14)
		nd, err1 := xml.Marshal(replyMsg)
		if err1 != nil {
			log.Errorf("marshal msg error " + err1.Error())
			err = nil
	...
	log.Errorf("checkUserInput ret %t, code %d, phoneNum %s", ok, code, phoneNum)
	...
	if !ok {
		...
		if code == SENDPHONECODE {
			...
			if !yes {
				...
				errcode, yes := checkUserInfo(phoneNum)
				if yes {
					hasGet, err1 := HasGetRedPacket(phoneNum)
					if err1 == nil {
						...
						if hasGet > 0 {
							...
							replyMsg.Content = getContent(13)
						}
					} else {
						//服务器出错
						replyMsg.Content = getContent(-1)
				...
				} else { // 非地大人或者 没有给歌手投过票

					content := ""
					if errcode == 0 {
						content = getContent(-1) // 内部服务器错误

					} else if errcode == 1 {
						content = getContent(10) //非地大用户

					} else if errcode == 2 {
						content = getContent(11) // 此次歌手大赛中用户未投票

					} else if errcode == 3 {
						content = getContent(12) // 投票的歌手未获得第一名
					}
...
func checkUserInfo(phoneNum string) (code int, ret bool) {
	...
	sql := fmt.Sprintf(`select uin, schoolId from profiles where phone = "%s"`, phoneNum)
	...
	for rows.Next() {
		rows.Scan(&uid, &schoolId)
	}

	if schoolId == 78629 {

		var singerId int
		sql = fmt.Sprintf(`select singerId from ddsingerFansFromPupu where uin = %d`, uid)
...
func doSendRedPacket(openId string, phoneNum string) (code int) {
	...
	hasGet, err := HasGetRedPacket(phoneNum)
	if err == nil {
		if hasGet == 1 {
			code = 3 // 用户已经领取过奖金
		} else {
			cnt, err := RecordSendRedPacket(openId, phoneNum)
			if err != nil {
				code = -1
			} else {
				if cnt == 0 {
				...
				money := cnt * 140
				log.Debugf("in doSendRedPacket openId:%s phoneNum:%s money:%d", openId, phoneNum, money)
...
func checkUserInput(openId, content string) (ok bool, code int, phoneNum string) {
	...
	if len(content) == 4 {
		...
		index := strings.Index(content, "我的歌声里")
		lent := len(content)

		log.Errorf("content:%s length %d, index(我的歌声里） %d", content, lent, index)

		//我的歌声里13590457127 = 26个字符
		if lent != 26 || index != 0 {
			// 输入文本格式不正确
...
func getContent(code int) (content string) {
	...
	switch code {
	case 1:
		content = "输入格式错误,请按如下格式输入：“我的歌声里”+“手机号”（注意:没有中间的加号哦）"
	case 2:
		content = "请输入地大女生节活动专属验证码"
	case 3:
		content = "你已经领取过奖金了哦～"
	case 4:
		content = "哎呦，不好意思，奖金派完了"
	case 5:
		content = "地大歌手大赛奖金领取专属验证码错误，请重新输入验证码"
	case 6:
		content = "地大歌手大赛奖金领取专属验证码已失效，请重新输入:“我的歌声里+“手机号”,获取新的验证码"
	case 7:
		content = "已经发送奖金给您，请您查收"
	case 8:
		content = "该手机号还没有注册【噗噗】，请先在应用商店下载并注册【噗噗】后，来找我领红包～（注意：注册要填写真实姓名）"
	case 9:
		content = "奖金领取活动已结束，感谢您的支持！有任何想法建议，欢迎发给我们！么么哒！"
	case 10:
		content = "此活动只针对地大用户！"
	case 11:
		content = "不好意思，您尚未参与我的歌声里投票活动！"
	case 12:
		content = "很可惜，你支持的选手离第一名只差一点，下次请继续加油哦～"
	case 13:
		content = "请输入您接收到的我的歌声里奖金领取专属验证码"
	case 14:
		content = "在应用商店搜索“噗噗”app，下载并注册成功后，来找我领红包～（注意：注册要填写真实姓名）"
	case 15:
		content = "只有为爱豆打call的用户才能领取红包哦～"
	default:
		content = "当前领取奖金人数过多！请稍后再试～"
	}
...
func sendPhoneCode(phone string, code int) (err error) {
	...
	req.Params = []string{"【噗噗】地大歌手大赛奖金领取验证码", text, "打死也不要告诉别人，3"}
