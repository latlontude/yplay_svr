...
func MakeStoryShareMsgWithText(fromUin int64, groupId, data, descStr string) (msg IMMsg, err error) {
	...
	log.Debugf("end MakeStoryShareMsgWithText")
	return
}

func SendStoryShareMsgWithText(fromUin, toUin int64, shareData, descStr string) (err error) {

	log.Debugf("start SendStoryShareMsgWithText fromUin:%d, toUin:%d", fromUin, toUin)
	...
	msg, err := MakeStoryShareMsgWithText(fromUin, groupId, shareData, descStr)
	if err != nil {
		log.Errorf(err.Error())
		return
	}

	log.Errorf("IMSendStoryShareMsgWithTextReq uin %d, req %+v", fromUin, msg)
	...
	log.Errorf("IMSendStoryShareMsgWithTextRsp uin %d, rsp %+v", fromUin, rsp)
	...
	log.Debugf("end SendStoryShareMsgWithText")
	return
}
