...
func SendV2CommonMsg(fromUin, toUin int64, dataType int, notifyData, descStr string) (err error) {
	...
	msg, err := MakeV2CommonMsg(fromUin, dataType, groupId, notifyData, descStr)
	if err != nil {
		log.Errorf(err.Error())
		return
	}
	...
	if err != nil {
		err = rest.NewAPIError(constant.E_IM_REQ_SEND_V2_COMMON_MSG, err.Error())
		log.Errorf(err.Error())
		return
	}
	...
	log.Debugf("end SendV2CommonMsg")
	return
}
