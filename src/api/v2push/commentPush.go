...
func GetV3Comment(commentId int) (comment st.CommentInfo, from int64, to int64, err error) {
	...
	sql := fmt.Sprintf(`select commentId ,answerId,commentContent,ownerUid,toUid,isAnonymous,AcommentTs 
from v2comments where commentId = %d`, commentId)
	...
	for rows.Next() {
		rows.Scan(&comment.CommentId, &comment.AnswerId, &comment.CommentContent, &from, &to, &comment.IsAnonymous, &comment.CommentTs)
	}

	if from > 0 {
		ui, err1 := st.GetUserProfileInfo(from)
		if err1 != nil {
			log.Error(err1.Error())
		}
	...
	if to > 0 {
		ui, err1 := st.GetUserProfileInfo(to)
		if err1 != nil {
			log.Error(err1.Error())
		}

		comment.ToOwnerInfo = ui
	}
