...
func doAuth(req *AuthReq, r *http.Request) (replyStr *string, err error) {
	...
	*/
	...
	if strings.Contains(recvMsg.Content, "注册") {
		...
		replyMsg.Content = getContent(11)
		nd, err1 := xml.Marshal(replyMsg)
		if err1 != nil {
			log.Errorf("marshal msg error " + err1.Error())
			err = nil
...
func checkUserInfo(phoneNum string) (ok bool) {
	...
	sql := fmt.Sprintf(`select schoolId from profiles where phone = "%s"`, phoneNum)
	...
	log.Debugf("end checkUserInfo ok:%t", ok)
	return
}
...
func doSendRedPacket(openId string, phoneNum string) (code int) {
	...
	if err == nil {
		if hasGet == 1 {
			...
			money, err := RecordSendRedPacket(openId, phoneNum)
			if err != nil {
				code = -1
			} else {
...
func checkUserInput(openId, content string) (ok bool, code int, phoneNum string) {
	...
	if len(content) == 4 {
		...
		index := strings.Index(content, "城院")
		lent := len(content)

		log.Errorf("content:%s length %d, index(城院） %d", content, lent, index)
...
func getContent(code int) (content string) {
	...
	switch code {
	case 1:
		content = "输入格式错误,请按如下格式输入：“城院”+“手机号”（注意:没有中间的加号哦）"
	case 2:
		content = "请输入城院注册【噗噗】领取红包专属验证码"
	case 3:
		content = "你已经领取过红包了哦～"
	case 4:
		content = "哎呦，不好意思，红包派完了"
	case 5:
		content = "城院注册【噗噗】领取红包专属验证码错误，请重新输入验证码"
	case 6:
		content = "城院注册【噗噗】领取红包专属验证码已失效，请重新输入:“城院”+“手机号”,获取新的验证码"
	case 7:
		content = "已经发送红包给您，请您查收"
	case 8:
		content = "该手机号还没有注册【噗噗】，请先在应用商店下载并注册【噗噗】后，来找我领红包～（注意：注册要填写真实姓名）"
	case 9:
		content = "红包领取活动已结束，感谢您的支持！有任何想法建议，欢迎发给我们！么么哒！"
	case 10:
		content = "此活动只针对城院用户！"
	case 11:
		content = "在应用商店搜索“噗噗”app，下载并注册成功后，来找我领红包～（注意：注册要填写真实姓名）"
	default:
		content = "当前领取红包人数过多！请稍后再试～"
	}
...
func sendPhoneCode(phone string, code int) (err error) {
	...
	req.Params = []string{"城院注册【噗噗】领取红包验证码", text, "打死也不要告诉别人，3"}
